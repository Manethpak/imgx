...
import { blobToDataUrl, copyBlobBase64, downloadBlob, formatExtension, formatLabel, reEncodeAs } from '../lib/image/export'
...
export function ExportDialog({ result, baseName, quality, onClose, onToast }: ExportDialogProps) {
  const dialogRef = useRef<HTMLDivElement>(null)
  const [selectedFormat, setSelectedFormat] = useState<ExportFormat>(result.type as ExportFormat)
  ...
  async function handleBase64() {
    ...
    try {
      ...
      onToast('Base64 generated')
    } catch {
      onToast('Base64 generation failed')
    } finally {
      setIsEncoding(false)
    }
  }
      ...
      <div className="export-dialog slide-up" ref={dialogRef}>
        ...
        <div className="format-picker">
          {exportFormats.map((fmt) => (
            <button
              ...
              onClick={() => { setSelectedFormat(fmt); setBase64Output('') }}
            >
              .{formatExtension(fmt)}
        ...
        {selectedFormat === 'image/x-icon' && (
          <p className="export-dialog__note">
            ICO will be resized to {Math.min(result.width, result.height, 256)}px (max 256px).
          </p>
        )}
